...
pub fn generate_protolang_code(model: &Model, meta: &[Meta]) -> String {
...
pub fn generate_protolang_code_type(type_def: &Type, meta: &[Meta]) -> String {
  let mut builder = String::new();
  if !meta.is_empty() {
    for item in meta {
      builder.push_str(&format!("meta {} = \"{}\";\n", item.key, item.value));
...
pub fn generate_protolang_code_enum(enum_def: &Enum, meta: &[Meta]) -> String {
  let mut builder = String::new();
  if !meta.is_empty() {
    for item in meta {
      builder.push_str(&format!("meta {} = \"{}\";\n", item.key, item.value));
